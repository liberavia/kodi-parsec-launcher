...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    ...
    _log("read_body_and_headers headers="+pprint.pformat(headers))
    _log("read_body_and_headers headers_length="+str(len(headers)))
    if len(headers)==0:
        headers.append(["User-Agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:18.0) Gecko/20100101 Firefox/38.0"])
    ...
    #_log("read_body_and_headers txheaders: " + pprint.pformat(txheaders) )
    req = Request(url, post, txheaders)
    req.add_header("User-Agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:18.0) Gecko/20100101 Firefox/38.0")
    
    # check if we need to add authentication header
    use_proxy         = get_setting("use_proxy")
    proxy_user          = get_setting("proxy_user")
    proxy_password      = get_setting("proxy_password")
    
    if use_proxy == "true" and proxy_user != "" and proxy_password != "":
        auth_encoded = base64.encodestring('%s:%s' % (proxy_user, proxy_password))[:-1]
        req.add_header('Authorization', 'Basic %s' % auth_encoded)
    
    #_log("read_body_and_headers build Request: " + pprint.pformat(req) )    
    if timeout is None:
        handle=urlopen(req)
    else:        
...
def simple_read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    _log("simple_read_body_and_headers "+url)

    if post is not None:
        _log("simple_read_body_and_headers post="+post)
        
    _log("simple_read_body_and_headers headers="+pprint.pformat(headers))
    _log("simple_read_body_and_headers headers_length="+str(len(headers)))
    if len(headers)==0:
        headers.append(["User-Agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:18.0) Gecko/20100101 Firefox/38.0"])

    # Start cookie lib
    ficherocookies = os.path.join( get_data_path(), 'cookies.dat' )
    _log("simple_read_body_and_headers cookies_file="+ficherocookies)
    ...
    # Let's see if cookielib is available
    try:
        _log("simple_read_body_and_headers importing cookielib")
        import cookielib
    except ImportError:
        _log("simple_read_body_and_headers cookielib no disponible")
        # If importing cookielib fails
        # let's try ClientCookie
        try:
            _log("simple_read_body_and_headers importing ClientCookie")
            import ClientCookie
        except ImportError:
            _log("simple_read_body_and_headers ClientCookie not available")
            # ClientCookie isn't available either
            urlopen = urllib2.urlopen
            Request = urllib2.Request
        else:
            _log("simple_read_body_and_headers ClientCookie available")
            # imported ClientCookie
            urlopen = ClientCookie.urlopen
            Request = ClientCookie.Request
            cj = ClientCookie.MozillaCookieJar()
        ...
        _log("simple_read_body_and_headers cookielib available")
        # importing cookielib worked
        urlopen = urllib2.urlopen
        Request = urllib2.Request
        cj = cookielib.MozillaCookieJar()
    ...
    # we successfully imported
    # one of the two cookie handling modules
        _log("simple_read_body_and_headers Cookies enabled")

        if os.path.isfile(ficherocookies):
            _log("simple_read_body_and_headers Reading cookie file")
            # if we have a cookie file already saved
            # then load the cookies into the Cookie Jar
            try:
                cj.load(ficherocookies)
            except:
                _log("simple_read_body_and_headers Wrong cookie file, deleting...")
                os.remove(ficherocookies)
        ...
        # for fetching URLs
        if cookielib is not None:
            _log("simple_read_body_and_headers opener using urllib2 (cookielib)")
            # if we use cookielib
            # then we get the HTTPCookieProcessor
            # and install the opener in urllib2
            if not follow_redirects:
            ...
            _log("simple_read_body_and_headers opener using ClientCookie")
            # if we use ClientCookie
            # then we get the HTTPCookieProcessor
            # and install the opener in ClientCookie
            opener = ClientCookie.build_opener(ClientCookie.HTTPCookieProcessor(cj))
    ...
    # Construye el request
    if post is None:
        _log("simple_read_body_and_headers GET request")
    else:
        _log("simple_read_body_and_headers POST request")
    
    # Añade las cabeceras
    _log("simple_read_body_and_headers ---------------------------")
    for header in headers:
        _log("simple_read_body_and_headers header %s=%s" % (str(header[0]),str(header[1])) )
        txheaders[header[0]]=header[1]
    _log("simple_read_body_and_headers ---------------------------")

    #_log("simple_read_body_and_headers txheaders: " + pprint.pformat(txheaders) )
    req = Request(url, post, txheaders)
    req.add_header("User-Agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:18.0) Gecko/20100101 Firefox/38.0")
    ...
    #_log("simple_read_body_and_headers build Request: " + pprint.pformat(req) )    
    if timeout is None:
        handle=urlopen(req)
    else:        
        #Disponible en python 2.6 en adelante --> handle = urlopen(req, timeout=timeout)
    ...
    info = handle.info()
    _log("simple_read_body_and_headers Response")

    returnheaders=[]
    _log("simple_read_body_and_headers ---------------------------")
    for header in info:
        _log("simple_read_body_and_headers "+header+"="+info[header])
        returnheaders.append([header,info[header]])
    handle.close()
    _log("simple_read_body_and_headers ---------------------------")
    ...
    # Tiempo transcurrido
    fin = time.clock()
    _log("simple_read_body_and_headers Downloaded in %d seconds " % (fin-inicio+1))
    _log("simple_read_body_and_headers body="+data)
